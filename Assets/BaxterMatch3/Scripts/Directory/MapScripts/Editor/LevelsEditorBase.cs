...
namespace Internal.Scripts.MapScripts.Editor
{
    public abstract class LevelsEditorBase : UnityEditor.Editor
    {
        ...
        protected void SetStarsEnabled(LevelCampaign levelCampaign, bool isEnabled)
        {
            levelCampaign.SetStarsEnabled(isEnabled);
            if (isEnabled)
                levelCampaign.SetStarsType(levelCampaign.StarsType);
            EditorUtility.SetDirty(levelCampaign);
