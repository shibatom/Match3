...
{
    public class PostImportHelper : AssetPostprocessor
    {
        ...
        private static void CheckTargets()
        {
            ...
            if (t.targets.FirstOrDefault() == null || t.targets.FirstOrDefault()?.defaultSprites.Length == 0)
                EditorUtility.DisplayDialog("Restart Unity", "To initialize new targets Please Restart Unity ", "OK");
        }
        ...
        static void SetScriptingDefineSymbols()
        {
            ...
            foreach (BuildTargetGroup _target in _buildTargets)
            {
                var unityAds = Resources.Load<UnityAdsHolder>("Scriptable/UnityAdsID");
