...
namespace Internal.Scripts.Items.Editor
{
    [CustomPropertyDrawer(typeof(SpawnObjCounter))]
    public class SpawnAmountGUIDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            EditorGUI.BeginProperty(position, label, property);
            var count = property.FindPropertyRelative("SpawnAmount");
            Item item = PropertyUtilities.GetParent(property) as Item;
            Rect r1 = position;
            r1.height = EditorGUIUtility.singleLineHeight;
            if (item.currentType == ItemsTypes.Gredient)
            {
