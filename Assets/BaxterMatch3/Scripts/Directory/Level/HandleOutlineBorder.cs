...
namespace Internal.Scripts.Level
{
    ...
    /// </summary>
    public class HandleOutlineBorder
    {
        private int maxRows;
        private int maxCols;
        private FieldBoard _fieldBoard;

        public HandleOutlineBorder(int maxRows, int maxCols, FieldBoard fieldBoard)
        {
        ...
        {
            Rectangle rectangle = _fieldBoard.GetSquare(col, row, true);
            if (rectangle.type != LevelTargetTypes.NONE)
            {
                if (row == 0 || col == 0 || col == maxCols - 1 || row == maxRows - 1)
                {
                    var outline = Object.Instantiate(_fieldBoard.outline3, rectangle.transform);
                ...
                {
                    //top left
                    if (_fieldBoard.GetSquare(col - 1, row - 1, true).type == LevelTargetTypes.NONE && _fieldBoard.GetSquare(col, row - 1, true).type == LevelTargetTypes.NONE &&
                        _fieldBoard.GetSquare(col - 1, row, true).type == LevelTargetTypes.NONE)
                    {
                        var outline = Object.Instantiate(_fieldBoard.outline3, rectangle.transform);
                        outline.transform.localPosition = Vector3.zero + Vector3.left * 0.015f + Vector3.up * 0.015f;
                        outline.transform.localRotation = Quaternion.Euler(0, 0, 180);
            ...
            {
                bool corner = false;
                if (_fieldBoard.GetSquare(col - 1, row, true).type != LevelTargetTypes.NONE && _fieldBoard.GetSquare(col, row - 1, true).type != LevelTargetTypes.NONE)
                {
                    var outline = Object.Instantiate(_fieldBoard.outline2, rectangle.transform);
                ...
                if (!corner)
                {
                    if (_fieldBoard.GetSquare(col, row - 1, true).type != LevelTargetTypes.NONE)
                    {
                        var outline = Object.Instantiate(_fieldBoard.outline1, rectangle.transform);
                    ...
                    if (_fieldBoard.GetSquare(col - 1, row, true).type != LevelTargetTypes.NONE)
                    {
                        var outline = Object.Instantiate(_fieldBoard.outline1, rectangle.transform);
